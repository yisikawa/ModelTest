...
VS_OUTPUT main(VS_INPUT input)
{
    ...
    float3 worldNormal = normalize(mul(boneNormal, (float3x3)matWorld));

    float  diffuse  = saturate(dot(worldNormal, -lightDir.xyz));
    float3 lighting = lightAmbient.rgb + diffuse * lightDiffuse.rgb;
